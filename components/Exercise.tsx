...
import type { ExerciseData, Question } from '../types';
import { CheckCircleIcon, XCircleIcon, DocumentTextIcon, RefreshIcon, InformationCircleIcon } from './Icons';

interface ExerciseProps {
    data: ExerciseData;
}

export const Exercise: React.FC<ExerciseProps> = ({ data }) => {
    const [answers, setAnswers] = useState<Record<string, string>>({});
    const [isSubmitted, setIsSubmitted] = useState<boolean>(false);
    const [score, setScore] = useState<number>(0);

    // Reset state when exercise data changes
    useEffect(() => {
        setAnswers({});
        ...
        <div>
            <header className="mb-8 border-b-2 border-primary dark:border-blue-400 pb-4">
                <h1 className="text-3xl md:text-4xl font-bold text-primary dark:text-blue-400 font-display flex items-center">
                    <DocumentTextIcon className="h-8 w-8 md:h-10 md:w-10 mr-3 md:mr-4"/>
                    {data.title}
                </h1>
            </header>
